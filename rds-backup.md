...
本文提供了手动部署的流程以及相关lambda的代码, 您可以按照该流程一步步创建所需资源。 同样，本文还提供了一个CloudFormation自动化部署脚本。该脚本可以快速自动完成部署，不需要您修改任何代码相关的部分，但相比起手动创建来说会多创建2个标准参数 （AWS System Manager服务中的Parameter store服务，具体说明参见下文）。
...
您可以通过点击下方 **Quick Start** 链接直接进入Cloudformation创建页面。该模板会自动创建上述手动部署流程中的各个资源。
...
- 在 **指定堆栈详细信息** 页面上，填写您的自定义参数，参数说明如下：
  ...
  - MaxSnapshotNumber: 您想保存最大的副本数量(最大100)
    ...
    **注意** ：在此您已自定义了相关参数，因此之后无需在lambda的代码界面修改相应代码。若您希望在资源创建后再次修改，请参考下文 **修改应用该脚本的RDS实例或者最大备份上限**
